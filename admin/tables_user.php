...
if (isset($_POST['tambah'])) {
  ...
  if ($cek == 0) {
    ...
    mysqli_query($conn, "INSERT INTO user VALUES(null, '$email', '$name', '$password', '$role', '$date_created', '$date_modified')");
    echo "<script>
              ...
              <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                ...
                <tbody>
                  ...
                  <tr class="text-center">
                      ...
                      <td><?= $user['name']?></td>
                      <td width="14%"><?=$user['role'];?></td>
                      <td width="5%"><?= $user['date_created']?></td>
                      <td width="5%"><?= $user['date_modified']?></td>
                      <td class="text-center"  >
                      <button type="button" class="btn btn-success mb-2"  data-toggle="modal" data-target="#modalUbah<?= $user['id_user'];?>">
                      <i class="fa fa-edit"></i> Edit</button>
                      <button type="button" class="btn btn-danger mb-2"  data-toggle="modal" data-target="#modalHapus<?= $user['id_user'];?>">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" name="tambah" class="btn btn-primary">Add User</button>
      </div>
      </form>
    </div>
  </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-success text-white">
        <h5 class="modal-title" id="exampleModalLabel">Update Data: <?= $user['name']?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
            ...
            <div class="mb-3">
                <label for="email" class="form-label">Email</label>
                <input type="email" class="form-control" value="<?= $user['email']; ?>" id="email" name="email" placeholder="Email Address" 
                Required disabled>
            </div>
...
<!-- Modal Hapus -->
<?php foreach ($data_user as $user) : ?>
<div class="modal fade" id="modalHapus<?= $user['id_user'];?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header bg-danger text-white">
          ...
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            ...
            <div class="modal-body">
                    <p>Yakin Ingin Menghapus Data User: <?= $user['name'];?> ?</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Kembali</button>
                <a href="hapus-akun.php?id_user=<?= $user['id_user'];?>"  class="btn btn-danger" >Hapus</a>
            </div>
        </div>
    </div>
</div>
